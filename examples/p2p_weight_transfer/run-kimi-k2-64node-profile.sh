...
# Multi-node (64-node) profiling script for Kimi-K2 with broadcasting/p2p weight transfer.
# 32 nodes for training, 32 nodes for rollout (non-colocated / disaggregated).
#
# Configuration sourced from tests/test_weight_transfer_moe_multinode_kimik2_64nodes.py
# on the rdma-weight-transfer-with-profiling branch.
#
# Usage:
#   bash run-kimi-k2-64node-profile.sh <MODE> <NODE_RANK> <HEAD_NODE_IP>
#
#   MODE          : broadcast | p2p
#   NODE_RANK     : 0 (head node) | 1..63 (worker nodes)
#   HEAD_NODE_IP  : IP address of the head node
#
# Examples:
#   bash run-kimi-k2-64node-profile.sh p2p 0 10.0.0.1   # head node
#   bash run-kimi-k2-64node-profile.sh p2p 1 10.0.0.1   # worker node
...
if [ $# -lt 3 ]; then
    ...
    echo "  NODE_RANK    : 0 (head) | 1..63 (workers)"
...
NNODES=64
GPUS_PER_NODE=8
NUM_TRAIN_GPUS=256    # 32 nodes
NUM_ROLLOUT_GPUS=256  # 32 nodes
...
run_mode() {
    ...
    # --- Training parallelism (from rdma branch test) ---
    PERF_ARGS=(
    ...
    # --- SGLang: 8 engines x 32 GPUs (from rdma branch test) ---
    SGLANG_ARGS=(
