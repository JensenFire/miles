"""
Multi-model 4-node profiling script for different methods of weight transfer.

Currently support models:
- llama3,glm4,glm45-air,moonlight,qwen3-30b,qwen3-32b
for NCCL weight transfer.
...
class ScriptArgs(U.ExecuteTrainConfig):
    ...
    relaxed_mc_transfer_timeout: bool = False
    no_save_optim: bool = False
    wait_for_launching: bool = False
...
# ---------------------------------------------------------------------------
def execute(args: ScriptArgs, cfg: ModelConfig, mode: str, is_last_mode: bool = True):
    assert mode == "nccl", "Currently only nccl weight transferring is supported."
    ...
    # for multinode scenarios, `mc_transfer_timeout` could be kinda relaxed
    mc_transfer_timeout = "300" if args.relaxed_mc_transfer_timeout else "30"
    ...
    if args.node_rank > 0 and args.wait_for_launching:
        if is_last_mode:
...
def main(args: ScriptArgs):
    ...
    for ci, cfg in enumerate(model_cfgs):
        ...
        for mi, mode in enumerate(modes):
            ...
            print(f"{'='*60}\n")
            execute(args, cfg, mode, is_last_mode=is_last)
